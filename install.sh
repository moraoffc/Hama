...
check_token() {
  ...
  if [ "$USER_TOKEN" = "mora" ]; then
    echo -e "${GREEN}AKSES BERHASIL${NC}}"
  else
    echo -e "${GREEN}Buy dulu Gih Ke ZeroDev${NC}"
    echo -e "${YELLOW}TELEGRAM : @moraoffc${NC}"
    echo -e "${YELLOW}WHATSAPP : 087732990717${NC}"
...
while true; do
  ...
  echo -e "${WHITE}ggg:     gg.     -   ,ggg       • WhatsApp : 6287732990717    ${NC}"
  echo -e "${WHITE} ggg:     ggg._    _,ggg        • Credit  : MoraBotz ${NC}"
  echo -e "${WHITE} ggg.    '.'''ggggggp           • Support by Mora Botz  ${NC}"
